var  request = require('request');
var fs = require('fs');
var zlib = require('zlib');
var tar = require('tar-fs');  
var filewalker = require('filewalker');
var rmdir = require ('rimraf');

function extractZip(gzFile,extractPath){
  fs.createReadStream(gzFile).
	   pipe(zlib.Unzip()).
	   pipe(tar.extract(extractPath));
};
    
function deleteTar(gzFile){
  rmdir(gzFile,function(er){
    if(er){
      console.log("Could not remove tarFile",gzFile,er);  
    }
  });
};

function getExtractedFileName(extractPath,callback){
  var dirName;
  filewalker(extractPath,{recursive: false})
    .on('dir', function(p) {
      dirName = p;
    })
    .on('error', function(err) {
      console.error(err);
      return callback(null);
    })
    .on('done', function() {
      if(this.dirs!=1){
        console.log("Seems there already exists extracted tar file");
        return callback("eems there already exists extracted tar file");
      }
      callback(null,extractPath+dirName);  
    })
  .walk();
};

function downloadRelease(URL,tarName,extractPath,calllback){
  // Path to where extracted zip files go
  //var extractPath = pathExtract+programName + '/';
  
  var options = {
      headers: {'user-agent': 'node.js'},
      tls: {
        rejectUnauthorized:false
      },
      uri: URL
  };

  var data = request(options)
    .pipe(fs.createWriteStream(tarName))
    .on('close', function () {
         extractZip(tarName,extractPath);
         getExtractedFileName(extractPath,calllback);
         deleteTar(tarName);
    });
};

module.exports = {
  'downloadRelease': downloadRelease
};

/*
   Another way of extracting files 
   var tar = require('tar');
   var mkdirp = require('mkdirp');
   var path = require('path');

   function decompress(tarball){
    fs.createReadStream(tarball)
        .on('error', console.log)
        .pipe(zlib.Unzip())
        .pipe(tar.Parse())
        .on('entry', function(entry) {
            var isDir     = 'Directory' === entry.type;
            var fullpath  = path.join(extractPath, entry.path);
            var directory = isDir ? fullpath : path.dirname(fullpath);
            mkdirp(directory, function(err) {
              if (err) throw err;
                if (! isDir) {
                    entry.pipe(fs.createWriteStream(fullpath));
              }
            });
        });
	
    };

*/
